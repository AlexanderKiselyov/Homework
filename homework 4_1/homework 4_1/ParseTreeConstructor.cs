...
namespace ParseTree
{
	/// has one public static method that builds a Parse Tree by sentence and returns its head
	public class ParseTreeConstructor
	{
		public static InterfaceParseTreeVertex ConstructParseTree(string sentence)
		{
		...
		private static InterfaceParseTreeVertex WorkRealSentence(string sentence)
		{
			int number = 0;
			...
			throw new ConstructionErrorException("Didn't manage...");
		}
		...
		private static void AddChildToStackTop(ParseStackElement top, InterfaceParseTreeVertex added)
		{
			if (top.GetCount() == 0)
			{
