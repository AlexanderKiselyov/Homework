...
int main()
{
	...
	int decimalSumOfNumbers = 0;
	for (int i = (sizeof(int) * 8 - 1); i >= 0; i--)
	{
		decimalSumOfNumbers += (massFirst[i] + massSecond[i])*currentDegree;
	...
	cout << "Десятичная запись суммы чисел: " << decimalSumOfNumbers << endl;
	return 0;
}
